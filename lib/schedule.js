...
const STEP_MINUTES = SLOT_MINUTES + BUFFER_MINUTES;
...
function buildBookableSlots({
  ...
  for (const window of assistedSlots) {
    ...
    if (remainder) cursor = new Date(cursor.getTime() + ((5 - remainder) * 60000));

    while (cursor < windowEnd && cursor <= latest) {
      const appointmentEnd = new Date(cursor.getTime() + (SLOT_MINUTES * 60000));
      const bufferedEnd = new Date(cursor.getTime() + (STEP_MINUTES * 60000));
