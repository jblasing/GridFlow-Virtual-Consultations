...
function mountTestConsole(app, deps) {
  const { pool, layout, graphToken, sendEmail, sendSms, resolveZuperApiBase,
    availableSlots, assignAndSchedule, addZuperNote, attachToZuper, brandonEmail, sendTestInvitation } = deps;
    ...
    return layout('Virtual consultation test console', `<section>
      ...
      <div class="test-grid">
        ...
        <form method="post" action="${action}"><h2>4. Real invitation email</h2>
          ...
          <p>Sends the production HTML email with a private scheduling link. No SMS is sent.</p><button>Send real invitation email</button></form>
        <form method="post" action="${action}"><h2>5. Test SMS</h2>
          <input type="hidden" name="testType" value="sms"><label>Recipient phone</label><input name="phone" required><button>Send test SMS</button></form>
        <form method="post" action="${action}"><h2>6. Zuper availability</h2>
        ...
        <form method="post" enctype="multipart/form-data" action="${action}"><h2>7. Checklist delivery</h2>
        ...
        <form method="post" action="${action}"><h2>8. Assignment and scheduling</h2>
    ...
    try {
      if (type === 'configuration') {
        const required = ['DATABASE_URL','BOOKING_TOKEN_SECRET','ADMIN_SECRET','INTEGRATION_SECRET',
          ...
          'MS_CLIENT_SECRET','MS_SENDER_EMAIL','TWILIO_ACCOUNT_SID','TWILIO_AUTH_TOKEN','TWILIO_FROM_NUMBER'];
        ...
        const phone = clean(req.body.phone); if (!phone) throw new Error('Recipient phone is required.');
        await sendSms(phone, 'Collaborative Services virtual consultation test. No appointment or lead was created.');
        detail = 'Test SMS accepted by Twilio.';
        ...
        await attachToZuper(jobUid, req.file);
